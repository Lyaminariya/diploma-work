...
from django.db.models import Sum, Avg, Count, F, FloatField, ExpressionWrapper, Q
...
class PlayerViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    """
    Эндпоинт API для общей статистики игрока.
    Доступен по /api/players/{player_id}/overall_stats/
    """
    ...
    def overall_stats(self, request, id=None):
        ...
        # Выбираем ID последних 20 матчей
        ...
        kda_ratio = safe_division(
            ...
            default=(total_kills + total_assists)
        ) # KDA по умолчанию, если смертей 0
        ...
        serializer.is_valid(raise_exception=True) # Проверяем, что данные соответствуют формату сериализотара
        return Response(serializer.validated_data)

    # Эндпоинт для истории матчей игрока
    # Доступен по /api/players/{player_id}/match-history/
